...
- **RustDesk Remote Desktop**: Install RustDesk with optional self-hosted relay server (rerunnable)
- **SSH Setup & Hardening**: Install and secure SSH: custom port, strong crypto, Keys, drop-in config, post-quantum KEX (rerunnable)
- **Tailscale VPN Setup**: Install and configure Tailscale VPN with custom hostname
- **Antigravity Fresh Refresh**: Fresh reset of Antigravity (clears cache/auth/tokens) while preserving all conversations, brain & memory
- **GitHub SSH Key Gen & Connect**: Auto-generates GitHub SSH keys, installs clipboard deps, and verifies connection
- **Hello World**: A simple test script to verify the custom tab works
- **Rahul's Alacritty Setup**: Install Alacritty with Rahul's Nordic theme config from dwm-rahul
- **Rahul's Architecture Setup**: Creates ~/projacts/ structure and clones .agent and rahul-second-brain
- **Rahul's DWM Setup**: Complete DWM environment setup with xinitrc (no SDDM) from dwm-rahul
- **Rahul's Fastfetch Setup**: Install Fastfetch with Rahul's Rajasthani pastel theme from mybash
- **Rahul's Ghostty Setup**: Install Ghostty with Rahul's config from dwm-rahul
