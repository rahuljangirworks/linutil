...
# If already running, focus the existing window instead of spawning a duplicate
APPIMAGE_PID="\$(pgrep -f "$APPIMAGE_PATH" 2>/dev/null | head -1 || true)"
if [ -n "\$APPIMAGE_PID" ]; then
  WINDOW_ID="\$(xdotool search --pid "\$APPIMAGE_PID" --name opencode 2>/dev/null | head -1 || true)"
  if [ -n "\$WINDOW_ID" ]; then
...
# --disable-gpu prevents GPU process crashes on Intel integrated graphics
# that can destabilise other GPU-accelerated apps (browser, etc.)
exec "$APPIMAGE_PATH" --disable-gpu "\$@"
EOF
