...
# Description: Setup Rahul's customized Fastfetch configuration
...
setupFastfetchConfig() {
    printf "%b\n" "${YELLOW}Setting up Rahul's Fastfetch config...${RC}"
    
    # Backup existing config if it exists and no backup exists yet
    if [ -d "${HOME}/.config/fastfetch" ] && [ ! -d "${HOME}/.config/fastfetch-bak" ]; then
        printf "%b\n" "${YELLOW}Backing up existing Fastfetch config...${RC}"
        cp -r "${HOME}/.config/fastfetch" "${HOME}/.config/fastfetch-bak"
    fi
    
    mkdir -p "${HOME}/.config/fastfetch/"
    
    # Download config from Rahul's mybash repo (Rajasthani pastel theme)
    printf "%b\n" "${CYAN}Downloading config from rahuljangirworks/mybash...${RC}"
    curl -sSLo "${HOME}/.config/fastfetch/config.jsonc" \
        "https://raw.githubusercontent.com/rahuljangirworks/mybash/main/config.jsonc"
    
    printf "%b\n" "${GREEN}Fastfetch configuration installed!${RC}"
    printf "%b\n" "${CYAN}Config location: ~/.config/fastfetch/config.jsonc${RC}"
}
