...
# Description: Install Linutil using the same flow as the upstream applications setup tab.
...
installExtra() {
    ...
    curl -fsSL 'https://raw.githubusercontent.com/ChrisTitusTech/linutil/refs/heads/main/man/linutil.1' |
        "$ESCALATION_TOOL" tee '/usr/share/man/man1/linutil.1' >/dev/null

    printf "%b\n" "${YELLOW}Creating a desktop entry...${RC}"
    "$ESCALATION_TOOL" mkdir -p /usr/share/applications
    curl -fsSL 'https://raw.githubusercontent.com/ChrisTitusTech/linutil/refs/heads/main/linutil.desktop' |
        "$ESCALATION_TOOL" tee /usr/share/applications/linutil.desktop >/dev/null
    printf "%b\n" "${GREEN}Done.${RC}"
}
